 ...
 * `-latest` 같은 **별칭**인가.
 *
 * ## 왜 별칭을 따로 식별하는가 (실측 2026-07-31·09-22)
 * 별칭은 이름일 뿐 실체가 아니다. API가 자기 실체를 밝히지 않으므로
 * (`gemini-3.6-flash`의 version은 `3.6-flash-07-2026`인데 `gemini-flash-latest`는
 * `Gemini Flash Latest` — 자기 이름 반복) 별칭이 가리키는 모델이 목록의 명시 모델과
 * 같은지 알 수 없다. 쿼터는 모델별로 분리되는데(PerProjectPerModel), 실체가 겹치면
 * 두 칸으로 보이는 것이 실제로는 한 칸이다.
 *
 * 그래서 별칭은 **한도 계산에서 세지 않는다.** 실행에서 빼는 것이 아니라 "있다고 치고
 * 예산을 부풀리지 않는" 것이다 — 낙관적인 추정치는 실행 전 경고를 무력화한다
 * (실측 2026-07-31: 화면에 300회, 실질 60~80회. 그 숫자를 믿고 진행한 실행이
 * 첫 청크부터 429를 맞았다).
 *
 * 판정은 접미사 하나로 좁게 둔다. 넓히면 실체가 있는 모델까지 예산에서 빠져 이번에는
 * 반대 방향으로 틀린 숫자가 된다.
 *
 * @param {string} name
 * @returns {boolean}
 */
export function isAliasModel(name) {
  return /-latest$/.test(String(name ?? '').toLowerCase());
}
