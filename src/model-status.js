 ...
 * 실패 관찰의 사유 문자열을 만든다.
 *
 * tpm만 초를 박는다 — 이 판정은 요청 크기에 달린 조건부 판정이라 "몇 초짜리 요청에서
 * 막혔는지"가 관찰의 일부다.
 *
 * ## 설정값(--chunk)이 아니라 **실패한 구간의 실제 길이**를 쓴다 (수정 2026-07-30)
 * 마지막 청크는 영상 끝에서 잘려 설정값보다 짧다(예: 480 설정에 289초 꼬리).
 * 289초에서 막힌 것을 `tpm-480`으로 적으면, 다음 `--chunk 300` 실행이 "300 < 480이니
 * 무관"으로 판단해 이미 289초에서 막혔던 모델을 다시 시도한다. 관찰은 실제로 본 값이어야
 * 그 위에 세운 조건부 판정이 참이 된다.
 *
 * @param {'404'|'rpd'|'tpm'} kind
 * @param {number} sec 실패한 구간의 실제 길이(초)
 * @returns {string}
 */
export function failureReason(kind, sec) {
  if (kind !== 'tpm') return kind;
  const n = Math.round(Number(sec));
  // 길이를 알 수 없으면 가장 작은 값으로 남긴다. 작을수록 모든 청크 크기에 해당하므로
  // (적용 조건이 `현재 청크 >= 기록된 초`) 관찰을 놓치는 쪽이 아니라 지키는 쪽으로 기운다.
  return `tpm-${Number.isFinite(n) && n > 0 ? n : 1}`;
}
