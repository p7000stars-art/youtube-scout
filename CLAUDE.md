...
>
> **개정 2026-09-22 (모델 목록 자동 모드, 머지·실전 미검증)**: 14장의 꼬리 편입 규칙과
> 15장의 MODELS 초기값이 아래처럼 바뀌었다. 요지는 **기본값 단일 모델 폐지** —
> 목록을 지정하지 않으면 조회된 가용 모델 전체를 신형 우선으로 세워 쓴다. 개정된 문단에
> `(개정 2026-09-22)`를 붙였고, 그 아래 원문은 무엇이 왜 바뀌었는지의 기록으로 남겼다.
>
